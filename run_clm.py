...
def main():
    ...
    def group_texts(examples):
        ...
        # }
        # return result
        for k, s in enumerate(examples["input_ids"]):
            if len(s) > 512:
                s = s[:512]
            else:
                s += [tokenizer.eos_token_id] * (512-len(s))
            examples["input_ids"][k] = s
        for k, s in enumerate(examples["attention_mask"]):
            if len(s) > 512:
                s = s[:512]
            else:
                s += [0] * (512-len(s))
            examples["attention_mask"][k] = s
            # print(tokenizer.convert_ids_to_tokens(s))
        # for s in examples["input_ids"]:
        #     print(len(s))
        examples["labels"] = examples["input_ids"].copy()
        # for key in examples:
        #     print(key, [len(s) for s in examples[key]], np.shape(examples[key]))
        return examples
